...
const sevColor = (s) => s==='HIGH'?C.red:s==='MEDIUM'?C.orange:C.gold;
...
export default function CommandCenter() {
  ...
  return (
    <div style={{height:'100vh',display:'flex',flexDirection:'column',backgroundColor:C.bg,color:C.text,overflow:'hidden',
      boxShadow:chaos?`inset 0 0 0 3px ${C.red}`:undefined,
      background:chaos?`radial-gradient(ellipse 80% 50% at 50% 0%,rgba(255,45,45,0.12),${C.bg})`:`radial-gradient(ellipse 80% 50% at 50% 0%,rgba(0,100,200,0.12),${C.bg})`}}>
      ...
      <div style={{height:54,backgroundColor:'rgba(10,22,40,0.95)',borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 16px',flexShrink:0,backdropFilter:'blur(12px)'}}>
        ...
        <Link to="/" style={{display:'flex',alignItems:'center',gap:9,textDecoration:'none'}}>
          ...
          <span style={{fontWeight:800,fontSize:14,letterSpacing:'0.12em',color:C.text}}>KAVACH<span style={{color:C.cyan}}>.AI</span></span>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <button onClick={triggerChaos} style={{background:chaos?`linear-gradient(135deg,${C.red},#8b0000)`:`linear-gradient(135deg,rgba(255,45,45,0.15),rgba(139,0,0,0.2))`,color:C.red,border:`1px solid ${C.red}`,padding:'5px 12px',borderRadius:6,fontWeight:700,fontSize:11,cursor:'pointer',letterSpacing:'0.06em',boxShadow:`0 0 14px rgba(255,45,45,0.25)`,transition:'all 0.15s'}}>
            ⚡ CHAOS MODE
          </button>
          <div style={{display:'flex',alignItems:'center',gap:4,fontSize:11,color:connected?C.green:C.muted}}>
            <span style={{width:7,height:7,borderRadius:'50%',background:connected?C.green:C.muted,display:'inline-block',boxShadow:connected?`0 0 6px ${C.green}`:'none'}}/>
            {connected?'LIVE':'...'}
          </div>
        </div>
      </div>
      ...
      <div style={{flex:1,display:'grid',gridTemplateColumns:'260px 1fr 280px',gap:10,padding:10,overflow:'hidden',minHeight:0}}>
        ...
        <div style={{...panel,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
            <span style={{fontSize:10,fontWeight:700,letterSpacing:'0.14em',color:C.cyan}}>LIVE PRIORITY FEED</span>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10,overflow:'hidden',minHeight:0}}>
          ...
          <div style={{...panel,padding:'14px 16px',flexShrink:0}}>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:'0.14em',color:C.cyan,marginBottom:14}}>🤖 AI INSIGHTS</div>
          ...
          {/* Resources */}
          <div style={{...panel,padding:'14px 16px',flexShrink:0}}>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:'0.14em',color:C.cyan,marginBottom:14}}>RESOURCE STATUS</div>
          ...
          <div style={{...panel,flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
            <div style={{padding:'10px 14px',borderBottom:`1px solid ${C.border}`,fontSize:10,fontWeight:700,letterSpacing:'0.14em',color:C.cyan,flexShrink:0}}>AGENT FEED</div>
      ...
      <style>{`
        ...
        .leaflet-popup-content-wrapper{background:#0f1f38;color:#e2f0ff;border:1px solid rgba(0,200,255,0.2)}
        .leaflet-popup-tip{background:#0f1f38}
      `}</style>
    </div>
  );
}
...
function IncCard({inc,sel,onClick}){
  ...
  return(
    <div onClick={onClick} style={{borderLeft:`3px solid ${col}`,borderRadius:'0 8px 8px 0',backgroundColor:sel?C.bg3:C.bg1,borderTop:`1px solid ${sel?col+'44':C.border}`,borderRight:`1px solid ${sel?col+'44':C.border}`,borderBottom:`1px solid ${sel?col+'44':C.border}`,padding:'9px 11px',marginBottom:7,cursor:'pointer',transition:'background 0.15s',animation:'fadeIn 0.3s ease'}}>
      ...
      <div style={{height:2,background:'rgba(0,200,255,0.08)',borderRadius:1,marginBottom:5}}>
        <div style={{height:'100%',width:`${Math.round(inc.accuracy*100)}%`,background:`linear-gradient(90deg,rgba(0,200,255,0.3),${col})`,borderRadius:1}}/>
      </div>
  ...
  return(
    <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:11}}>
      ...
      <div style={{display:'flex',gap:4,flexWrap:'wrap',marginTop:8}}>
        {(inc.keywords||[]).map(kw=><span key={kw} style={{fontSize:9,color:C.cyan,background:'rgba(0,200,255,0.1)',border:`1px solid rgba(0,200,255,0.2)`,padding:'2px 6px',borderRadius:3}}>{kw}</span>)}
      </div>
    </div>
  );
}
...
function LogLine({log}){
  const col=log.type==='TRIAGE'?C.cyan:log.type==='DISPATCH'?C.green:log.type==='WARNING'?C.orange:C.red;
