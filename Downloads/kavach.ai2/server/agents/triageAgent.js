...
- resourceNeeded: "ambulance" | "fire" | "police" | "multiple"`
      }]
    });
...
const fallback = (text) => {
  ...
  const isCollapse = /collapse|building|rubble|stuck|trapped/.test(t);
  ...
  if (isFire || isMedical || isAccident || isCollapse) accuracy += 0.10;
  if (hasLocation) accuracy += 0.08;
  if (hasScale)    accuracy += 0.06;
  accuracy = Math.min(0.97, accuracy);

  const type = isFire ? 'Fire' : isMedical ? 'Medical' : isAccident ? 'Accident' : isCollapse ? 'Collapse' : 'Emergency';
  const sev  = (isFire || isCollapse) ? 'HIGH' : (isMedical || isAccident) ? 'MEDIUM' : 'LOW';
  const res  = isFire ? 'fire' : isMedical ? 'ambulance' : 'police';
  ...
  return {
    ...
    followUpQuestion: accuracy < 0.85 ? 'Exact address kya hai? Aur kitne log phaṃse hain?' : null,
